...
client.get("foo_rand000000000000", function (err, reply) {
    logcfy(reply.toString()); // Will print `OK`
});

// This will return a Buffer since original key is specified as a Buffer
client.get(new Buffer("foo_rand000000000000"), function (err, reply) {
    logcfy(reply.toString()); // Will print `<Buffer 4f 4b>`
});
client.quit();
```
...
client.get("foo_rand000000000000", function (err, reply) {
    logcfy(err); // => 'The connection has already been closed.'
});
```
...
sub.on("message", function (channel, message) {
    logcfy("sub channel " + channel + ": " + message);
    msg_count += 1;
    if (msg_count === 3) {
        sub.unsubscribe();
        sub.quit();
...
client.multi()
    ...
    .exec(function (err, replies) {
        logcfy("MULTI got " + replies.length + " replies");
        replies.forEach(function (reply, index) {
            logcfy("Reply " + index + ": " + reply.toString());
        });
    });
```
...
client.monitor(function (err, res) {
    logcfy("Entering monitoring mode.");
});
client.set('foo', 'bar');

client.on("monitor", function (time, args, raw_reply) {
    logcfy(time + ": " + args); // 1458910076.446514:['set', 'foo', 'bar']
});
```
...
client.zadd(args, function (err, response) {
    if (err) throw err;
    logcfy('added '+response+' items.');

    // -Infinity and +Infinity also work
    var args1 = [ 'myzset', '+inf', '-inf' ];
    client.zrevrangebyscore(args1, function (err, response) {
        if (err) throw err;
        logcfy('example1', response);
