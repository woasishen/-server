...
SOCKETS.prototype.broadcast = function (eventId, message, self) {
    logcfy("broadcast", eventId, message);
...
module.exports = {
    ...
    init_socket: function (socket) {
        logcfy('one user connect   ' + new Date(Date.now()).toTimeString());
        ...
        socket.on('close', function () {
            ...
            logcfy('one user disconnect    ' + new Date(Date.now()).toTimeString());
        });

        socket.on('error', function (err) {
            logcfy('socket error:' + err);
        })
    }
};
